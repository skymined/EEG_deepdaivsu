...
class Classifier_per_time(nn.Module):
    def __init__(self, embed_dim, output_dim):
        ...
        self.linear1 = nn.Conv1d(
            in_channels=embed_dim, out_channels=embed_dim // 4, kernel_size=1
        )
        self.linear2 = nn.Conv1d(
            in_channels=embed_dim // 4, out_channels=3, kernel_size=1
        )
...
class Baseline_model(nn.Module):
    def __init__(self, in_channel, out_channel, input_dim, final_dim, num_heads, topk):
        ...
        self.self_attention = nn.MultiheadAttention(
            embed_dim=out_channel, num_heads=num_heads
        )
    ...
    def forward(self, x):
        ...
        sf_attention_output, _ = self.self_attention(
            final_embed, final_embed, final_embed
        )
